#!/usr/bin/env bash
set -euxo pipefail
CANDIDATE_SHA="${1:?candidate sha}"
CONTROL_BUNDLE_SHA="${2:?control bundle sha}"
ROOT=/srv/hoopvision
...
if [ ! -x "$CONTROL/venv/bin/python" ]; then python3 -m venv "$CONTROL/venv"; fi
...
"$CONTROL/venv/bin/python" /tmp/ensure_ppocrv6.py
